...
/*lint --e{529}*/
    ...
    BOOL   fff=0;
    void __far * fpp;
    extern gui_control __far * root1, __far * root2, __far * sys_monitor;
    ...
    welcome_start();
    startup_message("正在加载配置参数...");
    read_config();
    startup_message("启动完成, 请按ENTER键进入生产。");
    welcome_ended();
    ...
    for(;;){        
        ...
        switch(key){/*lint --e{527}*/
            ...
            case 't': case 'T':
                if(1){
                    draw_font(100, 100, "我是中国人, I am Jun!"
                        , 0, 0, 0);
                }
                break;
            default:
...
/*lint --e{529}*/
    ...
    for(;;){
        ...
        prt_string(67, 8, "%         ", 0);
        sprintf(ts, "OS-VER:   %s", os_get_version_string());
        prt_string(55, 14, ts, 0);
        prt_string(5,18, os_get_description(), 0);
        ...
        #endif
        os_mutex_unlock(task_test_mutex);
        TaskSleep(10L);
    }
}
...
/*lint --e{529}*/
    ...
    for(;;){
        ...
        sprintf(ts, "%s)", os_get_version_string());
        prt_string(72,24, ts, 0);
        prt_string(2, 25, "------------------------------------------------------------------------------", 0);
...
int round_robin_mutex = 0;

void __task demo_round_robin(void __far * data)
{
    long  iii=0L;
    int   i,j;
    i = CurrentTask;
    ...
    i-= 7;
    for(;;){
        prt_string(i*10+10, 10, "Task", 9);
        textprintdec16(i*10+14, 10, LockNesting, 10, 2|CHG_OPT_FIL);
        textprinthex32(i*10+9, 12, iii++, 0);
        if(j==8){
            TaskSleep(1000L);
        }
    }
...
void __far __task start2(void __far * data)
{
    KEYCODE key;
    int slice = 5;
    ...
    InitPLCService();
    demo_init_plc();
    TaskCreate(demo_round_robin, NULL, NULL, 1024, 7, 0);
    TaskCreate(demo_round_robin, NULL, NULL, 1024, 7, 0);
    TaskCreate(demo_round_robin, NULL, NULL, 1024, 7, 0);
    TaskCreate(demo_round_robin, NULL, NULL, 1024, 7, 0);
    TaskCreate(demo_round_robin, NULL, NULL, 1024, 7, 0);
    TaskCreate(TaskTest, (void __far *)12,  "Task4", 1512u, 3u, TASK_CREATE_OPT_NONE);
    TaskCreate(TaskTest, (void __far *)12,  "Task4", 1512u, 3u, TASK_CREATE_OPT_NONE);
    TaskCreate(TaskTest, (void __far *)12,  "Task4", 1512u, 3u, TASK_CREATE_OPT_NONE);
    TaskCreate(TaskTest, (void __far *)12,  "Task4", 1512u, 3u, TASK_CREATE_OPT_NONE);
    TaskCreate(TaskTest, (void __far *)12,  "Task4", 1512u, 3u, TASK_CREATE_OPT_NONE);
    ...
    while(1){
        ...
        if(key)
            SetTimeSlice(slice);
        textprintdec16(16, 8, slice, 11, 6|CHG_OPT_FIL);
    }
}
