...
void dpc_for_test(void * d, int nr)
{
    ...
    textprintdec32(20, 24, SecondsFromStart, 9, CHG_OPT_FIL|13);
...
void far TaskMain(void * data)
{
    ...
    TaskCreate(TaskDumpArp, (void*)3,"Task9", 512u, 4u, TASK_CREATE_OPT_NONE);
    DispatchUnlock();
...
void far TaskTest(void * data)
{
    ...
    for(;;){
        DispatchLock();
        prt_string(2, 24, "Link: ", 0);
        prt_string(8, 24, nic_islink()?"OK":"NO", 0);
        textprintdec32(12, 24, eth_stat.rx_packets, 0, 0);
        textprintdec32(17, 24, eth_stat.tx_packets, 0, 0);
        DispatchUnlock();
        TaskSleep(100L);
    }
}
...
void far TaskDumpArp(void * data)
{
    ...
    arp_initialize();
    if(TaskCreate(TaskNIC, NULL, "TaskDumpArp", 1024, 2, TASK_CREATE_OPT_NONE)){
    } else {
    ...
    for(;;){
        DispatchLock();
        prt_string(2, 2, "Arp Table: ------------------------------", 0);
        for(i=0; i<20; i++){
            memset(buf, ' ', 60);buf[60]=0;
            if(arp_table[i].timeout == 0){
                prt_string(2, 3+i, buf, 0);
                continue;
            }
