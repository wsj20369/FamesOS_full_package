...
void far uCOS_demo1_Task (void *data);
...
void far uCOS_demo1_TaskStart (void *data)
{
    INT08U  i;
    char   s[100];
    KEYCODE key;
    ...
    textprintstr(27,  1, "FamesOS, The Real-Time Kernel", 0x0F + 0x40 + 0x80);
    textprintstr(33,  2, "Wang Shujun, Jun", 0x0F);
    textprintstr(36,  4, "EXAMPLE #1", 0x0F);

    textprintstr(1, 23, "Determining  CPU's capacity ...", 0xF);
    InitStat();                                          /* Initialize uC/OS-II's statistics         */
    textprintstr(1, 23, "                               ", 0xF);

    for (i = 0; i < 10; i++) {                        /* Create N_TASKS identical tasks           */
        TaskCreate(uCOS_demo1_Task, (void *)i, "Task", 512, i + 1, TASK_CREATE_OPT_NONE);
    }
    textprintstr( 1, 23, "#Tasks          : xxxxx  CPU Usage: xxx %", 0xF);
    textprintstr( 1, 24, "#Task switch/sec: xxxxx", 0xF);
    textprintstr(29, 25, "<-PRESS 'ESC' TO QUIT->", 0x8F);
    SetKeyBufTask(CurrentTask);
    for (;;) {
        sprintf(s, "%5d", NumberOfTasks);                     /* Display #tasks running                    */
        textprintstr(19, 23, s, DISP_FGND_BLUE + DISP_BGND_CYAN);
        sprintf(s, "%3d", CPU_USED);                    /* Display CPU usage in %                    */
        textprintstr(37, 23, s, DISP_FGND_BLUE + DISP_BGND_CYAN);
        sprintf(s, "%5d", TaskSwitches);                    /* Display #context switches per second      */
        textprintstr(19, 24, s, DISP_FGND_BLUE + DISP_BGND_CYAN);
        TaskSwitches = 0;
        
        sprintf(s, "V%s", FamesOS_VersionString);   /* Display version number as Vx.yy          */
        textprintstr(75, 25, s, DISP_FGND_YELLOW + DISP_BGND_BLUE);
        GetDateTime(s);                                 /* Get and display date and time            */
        textprintstr(1, 25, s, DISP_FGND_BLUE + DISP_BGND_CYAN);

        if (key = GetKey()) {                     /* See if key has been pressed              */     
            if (key == ESC) {                             /* Yes, see if it's the ESCAPE key          */
                clrscr();
                ExitApplication();                            /* Return to DOS                            */
            }
        }
        TaskSleep(1000L);
    }
...
void far uCOS_demo1_Task (void *data)
{
    ...
    for (;;) {
        os_mutex_lock(task_sem);/* Acquire semaphore to perform random numbers        */
        x = random(80)+1;                          /* Find X position where task number will appear      */
        y = random(16)+1;                          /* Find Y position where task number will appear      */
        os_mutex_unlock(task_sem);                    /* Release semaphore                                  */
                                                 /* Display the task number on the screen              */
        textprintchar(x, y + 5, c, 0x07);    
        TaskSleep(5L);                            /* Delay 1 clock tick                                 */
    }
}
...
void APICAL InitApplication(void)
{
    ...
    IrqConnect(7, irq7);
    maintask=TaskCreate(uCOS_demo1_TaskStart, (void*)0, "Task2", 900u, 0u, TASK_CREATE_OPT_NONE);
}
